...
mem_init(void)
{
	...
	// Your code goes here:
	uint32_t required_memory = npages * sizeof(struct PageInfo);
	pages = boot_alloc(required_memory);
	memset(pages, 0, required_memory);
	...
	// Your code goes here:
	boot_map_region(
	        kern_pgdir, UPAGES, required_memory, PADDR(pages), PTE_U | PTE_P);
...
page_init(void)
{
	...
	size_t i;
	for (i = 1; i < npages; i++) {
		size_t pages_end = PADDR(boot_alloc(0));

		if (i < IOPHYSMEM / PGSIZE || i >= pages_end / PGSIZE) {
