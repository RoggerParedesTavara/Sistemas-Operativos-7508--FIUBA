...
mem_init(void)
{
	...
	// Your code goes here:
	uint32_t pages_required_memory = npages * sizeof(struct PageInfo);
	pages = boot_alloc(pages_required_memory);
	memset(pages, 0, pages_required_memory);

	//////////////////////////////////////////////////////////////////////
	// Make 'envs' point to an array of size 'NENV' of 'struct Env'.
	// LAB 3: Your code here.
	uint32_t envs_required_memory = NENV * sizeof(struct Env);
	envs = boot_alloc(envs_required_memory);
	memset(envs, 0, envs_required_memory);
...
page_init(void)
{
	...
	// Ocupado: [Page 0];[0x0A0000 , 0x100000];[JOS + array de pages]
	// Libre: [Memoria Baja];[Memoria Alta libre]

	size_t i;
	for (i = 1; i < npages; i++) {
		size_t envs_end = PADDR(boot_alloc(0));

		if ((i < IOPHYSMEM / PGSIZE || i >= envs_end / PGSIZE) &&
...
static void
boot_map_small_pg(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	pte_t *pgtable_entry = pgdir_walk(pgdir, (void *) va, 1);
	if (!pgtable_entry)
		return;
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	while (offset < size) {
#ifndef TP1_PSE
		boot_map_small_pg(pgdir, va + offset, size, pa + offset, perm);
		offset += PGSIZE;

#else
		if (((va + offset) % PTSIZE != 0) ||
		    ((pa + offset) % PTSIZE != 0) || (size < PTSIZE)) {
			boot_map_small_pg(
			        pgdir, va + offset, size, pa + offset, perm);
			offset += PGSIZE;
		} else {
			pte_t *pgdir_entry = pgdir + PDX(va + offset);
			*pgdir_entry = (pa + offset) | perm | PTE_P | PTE_PS;
			offset += PTSIZE;
		}
...
page_lookup(pde_t *pgdir, void *va, pte_t **pte_store)
{
	...
	struct PageInfo *page = pa2page(PTE_ADDR(*pgtable_entry));
...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	//
	// Your code here:
	size_t rounded_up_size = ROUNDUP(size, PGSIZE);
	if (base + rounded_up_size > MMIOLIM) {
		panic("Not enough memory to allocate IO region");
	}
	boot_map_region(
	        kern_pgdir, base, rounded_up_size, pa, PTE_W | PTE_PCD | PTE_PWT);
	base += rounded_up_size;
	return (void *) (base - rounded_up_size);
}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	if ((uintptr_t)(va + len) >= ULIM) {
		user_mem_check_addr =
		        (uintptr_t) va >= ULIM ? (uintptr_t) va : ULIM;
		return -E_FAULT;
	}
	uintptr_t rounded_down_va = ROUNDDOWN((uintptr_t) va, PGSIZE);
	uintptr_t rounded_up_end = ROUNDUP((uintptr_t)(va + len), PGSIZE);
	for (uintptr_t current_va = rounded_down_va; current_va < rounded_up_end;
	     current_va += PGSIZE) {
		pte_t *page_table_entry =
		        pgdir_walk(env->env_pgdir, (void *) current_va, 0);
		if (!page_table_entry ||
		    (*page_table_entry & (perm | PTE_P)) != (perm | PTE_P)) {
			user_mem_check_addr = current_va < (uintptr_t) va
			                              ? (uintptr_t) va
			                              : current_va;
			return -E_FAULT;
		}
